...
| M10 | Per-mesh material colour from the gz Visual | ✅ done | Commit `88d192bd`. `SubmitMeshes()` gives each mesh a unique `AZ::RPI::Material::Create` instance (StandardPBR base = basic_grey) and sets `baseColor.color` from `O3deMeshData::color` (already gathered from the Visual's diffuse) before `AcquireMesh`; the instance lives in `meshMaterials` keyed by id, released with the handle. Verified via the cyan `GZ_O3DE_DEMO_MESH_PIPE` box: telemetry `[gz-o3de] M10 mesh ... tint=(0.00,0.85,0.90) created=1 propValid=1 set=1 compile=1`. The tint is **not** visible in a screenshot — the demo's 800 cd overhead point light saturates the box's lit face to white and the sides are unlit (AuxGeom shapes show colour only because they bypass lighting); the material-property telemetry is the proof. Follow-ups: real per-vertex tangents (normal-mapped/textured materials), metallic/roughness/textures from `O3deMaterial` (still an M0 stub). |
