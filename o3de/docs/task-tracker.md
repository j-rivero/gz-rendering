...
| M9-B | Wire public `O3deScene::CreateMeshImpl` to a real Atom mesh | ⏳ next | Replace the M5-B no-op. Design: build the `ModelAsset` once at `CreateMeshImpl` time from `MeshDescriptor::mesh` (M9-A converter) and register it in the backend keyed by the mesh's gz id; add an `O3deMeshData` plain snapshot `{id, world pose, scale, color}` to the `RenderFrame*` pipeline; gather mesh geometries in `O3deRenderTarget::GatherFrame` (they currently fall through `ToBackendType` and are skipped); on the render thread `AcquireMesh` on first sight of an id, `SetTransform` each frame from the owning Visual's world pose, `ReleaseMesh` on disappearance — mirroring the id-keyed `SubmitLights()` handle lifecycle. Also wire `O3deSubMesh::SetMaterialImpl` to a per-mesh material colour. |
